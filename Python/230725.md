...
### set관련 메서드
- `.set(x)`
  - {}는 빈 딕셔너리이기 때문에 빈 set는 set()
-  `.remove(x)`
-  `.discard()`
   -  존재하지 않는 항목을 제거해도 에러x
   -  None 값을 반환하기 때문
- `.pop()`
  - 세트에서 **임의의** 요소를 제거하고 반환함
  - 세트는 순서가 없기 때문
  - 해시 테이블에 나타나는 순서대로 반환
  - 실행할 때 마다 다른 요소를 얻는다는 의미에서 무작위가 아님/
- 세트의 집합 메서드
  - 메서드와 연산자 모두 사용가능
  - .union()
    - 순서x
...
- `setdefault(key[,default])`
  - 반복문 없이 키 값 추가 가능
  - 처음 연결된 값을 반환하기 때문에 원래 있던 키는 값 변경되지 않음.
- `.update([other])`
  - 없는키는 새로 생성되고 기존키는 덮어씀
  - 여러개 동시에 넣기 가능
  - 마지막에 갱신된 값으로 덮어쓰기 됨
## 복사
